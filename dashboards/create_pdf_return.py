...
def fetch_invoice_data(invoice_number):
    ...
    cursor = conn.cursor()
    cursor.execute(
        "SELECT * FROM `orders_fattura_newfields` WHERE `id` = %s",
        (invoice_number,)
    )
...
class PDFReturn(FPDF):
    ...
    def invoice_header(self, invoice):
        ...
        end_y = self.draw_table([data], col_widths)  # Salva la posizione finale Y
    ...
    def product_table(self, products):
        ...
        self.set_y(self.get_y())  # Assicurati che inizi dopo la sezione precedente
        end_y = self.draw_table(data, col_widths, headers, bold_headers=True)  # Salva la posizione finale Y
        self.set_y(end_y)  # Imposta Y per la prossima sezione
...
def generate_return_pdf(order):
    ...
    def edit_and_confirm():
        ...
        popup = tk.Toplevel()
        popup.title("Modifica i dettagli della bolla di reso ")
        popup.geometry("1700x600")  # Set the size of the popup window
        ...
        tk.Label(data_frame, text="Mittente:", font=font_size).grid(row=4, column=0, **padding)
        entry_sender_name = tk.Entry(data_frame, width=entry_width, font=font_size)
        entry_sender_name.grid(row=4, column=1, **padding)
        entry_sender_name.insert(0, invoice["sender_name"])

        tk.Label(data_frame, text="Destinatario:", font=font_size).grid(row=5, column=0, **padding)
        entry_recipient = tk.Entry(data_frame, width=entry_width, font=font_size)
        entry_recipient.grid(row=5, column=1, **padding)
        entry_recipient.insert(0, invoice["recipient"])

        tk.Label(data_frame, text="Quantità Totale:", font=font_size).grid(row=6, column=0, **padding)
        entry_total_quantity = tk.Entry(data_frame, width=entry_width, font=font_size)
        entry_total_quantity.grid(row=6, column=1, **padding)
        entry_total_quantity.insert(0, invoice["total_quantity"])

        tk.Label(data_frame, text="Importo IVA:", font=font_size).grid(row=7, column=0, **padding)
        entry_iva_amount = tk.Entry(data_frame, width=entry_width, font=font_size)
        entry_iva_amount.grid(row=7, column=1, **padding)
        entry_iva_amount.insert(0, invoice["iva_amount"])

        tk.Label(data_frame, text="Imponibile:", font=font_size).grid(row=8, column=0, **padding)
        entry_total_amount = tk.Entry(data_frame, width=entry_width, font=font_size)
        entry_total_amount.grid(row=8, column=1, **padding)
        entry_total_amount.insert(0, invoice["total_amount"])

        tk.Label(data_frame, text="Totale Fattura EURO:", font=font_size).grid(row=9, column=0, **padding)
        entry_total_invoice_euro = tk.Entry(data_frame, width=entry_width, font=font_size)
        entry_total_invoice_euro.grid(row=9, column=1, **padding)
